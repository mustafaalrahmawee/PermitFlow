...
- ext 2a → HTTP `403` when a staff member calls the administrative summary seam (`GET /api/reporting/admin-summary`, the UC-14 endpoint carried here for self-containment); the staff summary itself is *limited*, not denied: HTTP `200` whose aggregates are verified by response-body assertion to cover only requests where the actor is the responsible staff member; expected DB effect: none. [derived from 03_use-cases.md UC-13 ext 2a + BR-015 + BR-016 + docs/conventions.md API error responses — fragile: rests on the scope reading flagged in Authorization]
